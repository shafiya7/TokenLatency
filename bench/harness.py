...
# #     if not values:
# #         return {"count": 0, "mean": float("nan"), "median": float("nan"), "min": float("nan"), "max": float("nan"), "p95": float("nan")}
# #     vals = sorted(values)
# #     n = len(vals)
...
# #     kv_rw_ms: float,
# # ) -> dict[str, float]:
# #     """Convert overlapping hook totals into a Goal-2-friendly component breakdown.

# #     Notes:
# #     - attention_total_ms overlaps with qkv and o_proj, so softmax is derived.
# #     - decoder_layer_total_ms overlaps with attention/mlp/layernorm, so residual is derived.
# #     - framework_overhead is the leftover after non-overlapping components are accounted for.
# #     """
...
# #             )
# #             kv0_transfer_ms = estimate_transfer_time_ms(kv0["kv_total_bytes"], self.cfg.estimated_bandwidth_gbps) or 0.0
# #             kv0["estimated_transfer_ms"] = estimate_transfer_time_ms(kv0["kv_total_bytes"], self.cfg.estimated_bandwidth_gbps)
...
# #                     kv_cache_bytes_per_token=self.kv_cache_bytes_per_token if self.cfg.use_kv_cache else None,
# #                 )
# #                 kv_transfer_ms = estimate_transfer_time_ms(kv_rw["kv_total_bytes"], self.cfg.estimated_bandwidth_gbps) or 0.0
# #                 delta = _finalize_breakdown(
# #                     raw=raw_delta,
# #                     total_step_ms=step_done - step_start,
# #                     sampling_ms=step_done - t_sample,
# #                     kv_rw_ms=kv_transfer_ms,
# #                 )
...
# #                 kv_rw["estimated_transfer_ms"] = estimate_transfer_time_ms(kv_rw["kv_total_bytes"], self.cfg.estimated_bandwidth_gbps)
...
# #             "runtime": {"device": str(self.device), "dtype": str(self.dtype).replace("torch.", "")},
...
#                 "kv_cache_measurement_note": "KV-cache transfer time is estimated analytically from bytes moved and configured bandwidth; it is not directly isolated by a hardware profiler.",
...
class HFLatencyHarness:
    def __init__(self, cfg: BenchmarkConfig):
        ...
        try:
            ...
            # Compatibility fallback for transformers versions that still expect `dtype`.
